...
# 🚀 1. ページ構成・UI設定
# ======================================================
st.set_page_config(page_title="Kanaloa AI Pro Ver13.2", layout="wide")
...
st.title("🏇 Kanaloa AI Pro Ver13.2")
st.caption(f"システム稼働時刻: {datetime.now().strftime('%Y/%m/%d %H:%M')} | 展開因子完全分離 ＆ 学習強化モデル")
...
NICK_SCORES = {
    ("キタサンブラック", "芝"): 40, ("オルフェーヴル", "芝"): 35,
    ("ミッキーアイル", "ダート"): 45, ("ジャスタウェイ", "ダート"): 50,
    ("リアルスティール", "ダート"): -50
}
    ...
    if os.path.exists(HISTORY_FILE) and os.path.getsize(HISTORY_FILE) > 0:
        ...
        for col in REQUIRED_COLUMNS:
            if col not in df.columns:
                df[col] = "未設定" if col in ['馬主', '人気ランク', '厩舎', '騎手', '備考'] else (0.0 if any(x in col for x in ['期待値', '推奨額', '払戻', 'オッズ']) else "")
                modified = True
        if modified:
            try: df.to_csv(HISTORY_FILE, index=False, encoding='utf-8-sig')
            except: pass
        return df
    return pd.DataFrame(columns=REQUIRED_COLUMNS)
...
def save_history(df):
    try: df.to_csv(HISTORY_FILE, index=False, encoding='utf-8-sig')
    except Exception: st.error(f"⚠️ 保存失敗: CSVが別のアプリで開かれている可能性があります。")

def get_adaptive_weights():
    weights = base_weights.copy()
    df_h = load_history()
    if df_h.empty: return weights
    try:
        df_h['着順_num'] = pd.to_numeric(df_h['着順'], errors='coerce')
        for f in weights.keys():
            pat = f"(?<!母){re.escape(f)}" if f == "父カナロア" else re.escape(f)
            rel = df_h[df_h['適用要素'].str.contains(pat, na=False, regex=True)].dropna(subset=['着順_num'])
            if len(rel) >= 5:
...
def calculate_investment(ai_score, popular_rank, odds, f_odds, top_odds, bankroll, head_count):
    if head_count <= 10 or odds <= 0.0: return 0.0, 0.0, 0.0, 0
    
    if top_odds >= 4.0: d_rate = {"A": 0.22, "B": 0.13, "C": 0.10, "D": 0.07, "E": 0.04}
    elif top_odds <= 2.2: d_rate = {"A": 0.38, "B": 0.15, "C": 0.06, "D": 0.02, "E": 0.01}
    else: d_rate = {"A": 0.31, "B": 0.18, "C": 0.10, "D": 0.05, "E": 0.02}
    ...
    bet = int(min(bankroll * (edge/(odds-1.0))*0.5, bankroll * 0.05) // 100) * 100 if edge > 0 else 0
...
# --- Tab 1: AI診断 ---
with tab1:
    if is_small_race: st.warning("⚠️ 10頭以下のレースは自動的にC判定（除外）となります")
    ...
    for i in range(st.session_state.h_count):
        with st.expander(f"🐴 馬 {i+1}", expanded=True):
            ...
            st.markdown("##### 📈 成長サイン ＆ 映像評価 (乗算ブースト発動)")
            ...
            movie_eval = c18.selectbox("映像評価", ["標準", "好走(不利なし)", "不利あり"], key=f"mv_{i}")

            if name: user_inputs.append({'名前':name, '馬主':owner, 'タイプ':typ, '性別':sex, '前走距離':p_dist, '枠':waku, '騎手':jock, '厩舎':trai, '母父':msire, '人気':rank, '単勝':odds, '複勝':f_odds, 'tp':tp, 'up':up, 'g1':g1, 'g2':g2, 'g3':g3, '映像評価':movie_eval})
    ...
    if st.button("🚀 AI診断実行", type="primary", use_container_width=True):
        if not user_inputs: st.warning("データを入力してください")
        else:
            weights = get_adaptive_weights()
            new_recs = []
            for h in user_inputs:
                score = 80; factors = [h['タイプ']]
                
                is_road = "ロード" in h['馬主']
                is_kanaloa_blood = ("カナロア" in h['タイプ']) or ("カナロア" in h['母父'])
                if is_road and r_track == "ダート" and is_kanaloa_blood:
                    if h['性別'] in ["牡", "セ"]: score += 20; factors.append("ロード×カナロア(牡/セ)ダート(+20)")
                    elif h['性別'] == "牝": score += 10; factors.append("ロード×カナロア(牝)ダート(+10)")
                
                for (sire, track), bonus in NICK_SCORES.items():
                    if sire in h['母父'] and track in r_track: score += bonus; factors.append(f"血統特注({bonus})")
                ...
                # 💡 展開・枠の判定（AI学習用のキーも同時に記録）
                if h['tp'] == "15": 
                    score += weights.get("テンP15", 20) + p_adj
                    factors.append("テンP15") # AI学習用キー
                    if h['枠'] <= 3: 
                        score += weights.get("内枠ボーナス", 8)
                        factors.append("内枠ボーナス") # AI学習用キー
                    
                    if p_adj > 0: factors.append(f"逃げ有利(+{p_adj}pt)")
                    elif p_adj < 0: factors.append(f"逃げ不利({p_adj}pt)")
                    else: factors.append("逃げ(展開平均)")
                    
                elif h['tp'] == "30":
                    score += weights.get("テンP30", 8) + int(p_adj/2)
                    factors.append("テンP30") # AI学習用キー
                    if p_adj > 0: factors.append(f"先行有利(+{int(p_adj/2)}pt)")
                    elif p_adj < 0: factors.append(f"先行不利({int(p_adj/2)}pt)")
                    else: factors.append("先行(展開平均)")
                    
                elif h['tp'] == "50":
                    if p_adj < 0:
                        score += int(abs(p_adj)/2); factors.append(f"差し有利(+{int(abs(p_adj)/2)}pt)")

                if '芝' in r_track and h['up'] == '15':
                    score += weights.get("上がりP15", 15)
                    factors.append("上がりP15") # AI学習用キー
                    if p_adj < 0:
                        score += int(abs(p_adj)/2); factors.append(f"豪脚有利(+{int(abs(p_adj)/2)}pt)")
                    else:
                        factors.append("豪脚(通常)")
                
                # 💡 黄金血統（AI学習用の完全一致キーに修正）
                if "ディープ" in h['母父'] and '芝' in r_track: score += weights.get("黄金A(ディープ)", 35); factors.append("黄金A(ディープ)")
                if "クロフネ" in h['母父'] and 'ダート' in r_track: score += weights.get("黄金B(クロフネ)", 30); factors.append("黄金B(クロフネ)")
                if "ダイワメジャー" in h['母父']: score += weights.get("黄金C(ダイワメジャー)", 5); factors.append("黄金C(ダイワメジャー)")
                
                for k in weights.keys():
                    if k in h['騎手'] or k in h['厩舎']: score += weights[k]; factors.append(k)
                
                if "カナロア" in h['タイプ'] and h['性別'] == "牝" and r_track == "ダート" and r_dist >= 1700: 
                    score -= 25; factors.append("カナ牝ダ中距離(-25)")

                multiplier = 1.0
                growth_count = sum([h['g1'], h['g2'], h['g3']])

                if h['映像評価'] == "好走(不利なし)": multiplier *= 1.15; factors.append("映像ブースト")
                if growth_count >= 2: multiplier *= 1.10; factors.append("成長ブースト")
                if h['人気'] in ["C","D","E"]: multiplier *= 1.10; factors.append("穴馬ブースト")
                if h['映像評価'] == "好走(不利なし)" and growth_count >= 2: multiplier *= 1.10; factors.append("映像×成長シナジー")

                multiplier = min(multiplier, 1.5)
                final_score = int(80 + ((score - 80) * multiplier))

                prob, w_ev, p_ev, bet = calculate_investment(final_score, h['人気'], h['単勝'], h['複勝'], top_odds, bank_total, r_heads)
                
                if is_small_race: judge = "C (少頭数除外)"
                else: judge = "A" if p_ev >= 115 and w_ev >= 125 else "B" if w_ev >= 110 else "C"
                
                st.write(f"### {h['名前']} [{judge}] 期待値: 単{w_ev:.1f}% / 複{p_ev:.1f}%")
                st.caption(f"馬主: {h['馬主']} | スコア詳細: 基礎80 + 補正{score-80} × 乗算{multiplier:.2f}倍 ＝ 最終 {final_score}pt | 適用要素: {', '.join(factors)}")
                if bet > 0: st.success(f"推奨投資額: ¥{bet:,} (単複各 ¥{bet//2:,} を推奨)")
                
                new_recs.append({'日付':r_date, '場':r_place, 'レース名':r_num, '馬名':h['名前'], '馬主':h['馬主'], '投資判定':judge, '予想期待値':w_ev, '複勝期待値':p_ev, '推奨額':bet, '適用要素':"|".join(factors), '人気ランク':h['人気'], '単勝オッズ':h['単勝'], '複勝オッズ':h['複勝'], '着順':"", '単勝払戻':0, '複勝払戻':0, '備考':""})
            
            df_old = load_history()
            save_history(pd.concat([df_old, pd.DataFrame(new_recs)], ignore_index=True).drop_duplicates(subset=['日付', '馬名'], keep='last'))
            st.success("ログ保存完了。分析タブで確認してください。")

# --- Tab 2: 結果入力 ---
with tab2:
    df_res = load_history()
    if df_res.empty: st.info("データがありません")
    else:
        pending = df_res[df_res['着順'].astype(str).str.strip().isin(["", "未設定", "nan"])].copy()
        if pending.empty: st.success("全結果入力済みです")
        else:
...
with tab3:
    ...
    if not df_anl.empty:
        ...
        if search_name: 
            ...
            display_df = display_df[display_df['馬名'].str.contains(s_word, na=False) | display_df['馬主'].astype(str).str.contains(s_word, na=False)]
        
        if filter_grade: 
            display_df = display_df[display_df['投資判定'].isin(filter_grade)]
        ...
        if not df_ok.empty:
            ...
            grade_stats = []
            for g in ["A", "B", "C"]:
                sub = df_ok[df_ok['投資判定'].str.startswith(g)]
                if len(sub) > 0:
                    inv = sub['投資'].sum()
                    rec = sub['回収'].sum()
                    grade_stats.append({'判定': g, '件数': len(sub), '複勝率': f"{(sub['着順_num']<=3).mean()*100:.1f}%", '回収率': f"{(rec/inv*100):.1f}%" if inv>0 else "0.0%"})
            if grade_stats: st.table(pd.DataFrame(grade_stats))

            st.subheader("💡 因子別複勝パフォーマンス")
            f_res = []
            
            def extract_core_factor(factor_str):
                cleaned = re.sub(r'\([+-]?\d+pt\)', '', factor_str)
            ...
            for core_f in sorted(all_core_factors):
                if core_f in invalid_factors: continue
